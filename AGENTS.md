...
`bstyle!` 和 `style_runtime` 必须使用同一套 `core::parse_classes` 规则，并对成功展开的相同样式产生等价的最终 `ResolvedScene`：写入相同的 Component、字段和值，包含默认字段语义、组件数量和 `Node` 是否存在。两者的计算时机不同：`bstyle!` 在编译期展开，错误应表现为编译错误；`style_runtime` 在 Scene resolve 期间展开，错误应直接 `panic!`，不能静默跳过。新增或修改样式时必须同时验证两种入口及其错误行为。

运行时和过程宏可以使用不同的 Rust 生成机制，但不能因此改变场景结果。重复的属性映射和默认值处理应集中在易读的公共逻辑中；如果现有实现出现两套重复转换，应在保证功能完全不变的前提下合并，优先选择可读性和结果一致性。
...
- `bstyle!` 与 `style_runtime` 对成功样式产生等价的最终场景结果；运行时展开错误直接 panic
